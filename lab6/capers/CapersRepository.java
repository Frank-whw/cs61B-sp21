...
public class CapersRepository {
    ...
    public static void setupPersistence() {
        CAPERS_FOLDER.mkdir();//creak the folder of .capers
    ...
    public static void writeStory(String text) {
        ...
        if (!storyFile.exists()) {//storyFile不存在
            newStoryContent = text;
        } else {
            String storyContent = readContentsAsString(storyFile);
            newStoryContent = storyContent + "\n" + text;
        }
        writeContents(storyFile, newStoryContent);//因为writeContents是覆写而不是在末尾添加
    ...
    public static void makeDog(String name, String breed, int age) {
        ...
        newDog.saveDog();//保存
        //newDog.toString();//无意思
        System.out.println(newDog);//println will use toString implicitly
    }
